...
class KnapsackEnv(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.randomize_params_on_reset = True
...
def predict_without_last_action(action_probs,model, obs, last_action,first):
    if first:
        obss = model.policy.obs_to_tensor(obs)[0]
        dis = model.policy.get_distribution(obss)
        probs = dis.distribution.probs
        action_probs = probs.detach().numpy() 
        
        
        # Set the probability of the last action to zero
        
        
        action_probs = action_probs[0]
        action_probs=np.array(action_probs)
    # Sample an action from the modified probabilities
    action_probs[last_action]=0.0
    new_action = action_probs.argmax()
    print(action_probs)
...
obs = RESET(env)
obs = np.reshape(obs, (env.N*3+3,))   
action_probs=np.zeros(env.N)
states=None
last_action = None
ignore=False
first=True
...
while i<100:
    if ignore:
        action,action_probs = predict_without_last_action(action_probs,model, obs, last_action,first)
        first=False
    else:    
